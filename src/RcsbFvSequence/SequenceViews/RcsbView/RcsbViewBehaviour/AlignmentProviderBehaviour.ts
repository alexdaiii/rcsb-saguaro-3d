...
export class AlignmentProviderBehaviour implements RcsbViewBehaviourInterface {
  ...
  observe(
    ...
    this.subscription = stateManager.subscribe<
      "model-ready",
      AlignmentDataType
    >(async (o) => {
...
async function loadNextModel(
  ...
  if (!alignments || !alignments.target_alignment) return;
  if (data.who == "user") return;
  const pdb = data.pdb;
  const targetAlignment = data.targetAlignment;
  const index = alignments.target_alignment.findIndex(
    (ta) =>
      ta?.target_id ==
  ...
  if (
    ...
    index < 0 ||
    index == alignments.target_alignment.length - 1
  )
    return;
