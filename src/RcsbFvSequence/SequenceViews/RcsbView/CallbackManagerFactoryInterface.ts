...
export abstract class AbstractCallbackManager<U>
  implements CallbackManagerInterface<U>
{
  ...
  private readonly isInnerSelection: DataContainer<boolean> =
    new DataContainer<boolean>();
  ...
  public async structureViewerSelectionCallback(
    mode: "select" | "hover",
  ): Promise<void> {
    if (this.rcsbFvContainer.get() == null) return;
    this.isInnerSelection.set(true);
    await this.innerStructureViewerSelectionChange(mode);
    this.isInnerSelection.set(false);
  }

  public pfvSelectionChangeCallback(
    selection: Array<RcsbFvTrackDataElementInterface>,
  ): void {
    if (this.isInnerSelection.get()) return;
    this.innerPfvSelectionChange(selection);
  }

  public abstract featureClickCallback(
    e: RcsbFvTrackDataElementInterface,
  ): void;
  public abstract highlightHoverCallback(
    selection: RcsbFvTrackDataElementInterface[],
  ): void;
  public abstract modelChangeCallback(
    defaultAuthId?: string,
    defaultOperatorName?: string,
  ): Promise<void>;
  public abstract pfvChangeCallback(args: U): Promise<void>;
  protected abstract innerStructureViewerSelectionChange(
    mode: "select" | "hover",
  ): Promise<void>;
  protected abstract innerPfvSelectionChange(
    selection: Array<RcsbFvTrackDataElementInterface>,
  ): void;
}
