...
export interface RcsbViewInterface<T, U> {
  ...
  additionalContent?(
    props: RcsbViewInterface<T, U> & AbstractViewInterface,
  ): JSX.Element;
...
export class RcsbView<T, U> extends AbstractView<RcsbViewInterface<T, U>, {}> {
  private boardConfigContainer: DataContainer<
    Partial<RcsbFvBoardConfigInterface>
  > = new DataContainer();
  private rcsbFvContainer: DataContainer<RcsbFvModulePublicInterface> =
    new DataContainer<RcsbFvModulePublicInterface>();
  ...
  constructor(props: RcsbViewInterface<T, U> & AbstractViewInterface) {
    ...
    this.props.rcsbViewBehaviour?.observe(
  ...
  additionalContent(): JSX.Element {
    return this.props.additionalContent ? (
      this.props.additionalContent(this.props)
    ) : (
      <></>
    );
  }
  ...
  async componentDidMount(): Promise<void> {
    ...
    this.boardConfigContainer.set({
      ...
      elementClickCallback: (e?: RcsbFvTrackDataElementInterface) => {
        ...
        if (
          typeof this.props.additionalConfig?.boardConfig
            ?.elementClickCallback === "function"
        )
      ...
      selectionChangeCallback: (
        ...
        if (
          ...
          this.props.additionalConfig?.boardConfig.selectionChangeCallback(
            selection,
          );
      },
      highlightHoverCallback: (
        ...
        if (
          typeof this.props.additionalConfig?.boardConfig
            ?.highlightHoverCallback === "function"
        )
          this.props.additionalConfig?.boardConfig.highlightHoverCallback(
            selection,
          );
      },
    });
  ...
  async updateDimensions(): Promise<void> {
    const trackWidth: number = this.getwidth();
    this.boardConfigContainer.set({
      ...this.boardConfigContainer.get(),
      trackWidth,
    });
    const select = this.rcsbFvContainer.get()?.getFv().getSelection("select");
    const dom = this.rcsbFvContainer.get()?.getFv().getDomain();
    await this.rcsbFvContainer
      .get()
      ?.getFv()
      .updateBoardConfig({boardConfigData: {trackWidth: trackWidth}});
    if (select)
      this.rcsbFvContainer
        .get()
        ?.getFv()
        .setSelection({
          elements: select.map((s) => ({
  ...
  async modelChangeCallback(
    defaultAuthId?: string,
    defaultOperatorName?: string,
  ): Promise<void> {
    await this.callbackManager.modelChangeCallback(
      defaultAuthId,
      defaultOperatorName,
    );
  }
  ...
  private getwidth(): number {
    ...
    return (
      width -
      (this.props.additionalConfig?.boardConfig?.rowTitleWidth ?? 190) -
      (this.props.additionalConfig?.boardConfig?.disableMenu ? 10 : 55)
    );
  }
}
