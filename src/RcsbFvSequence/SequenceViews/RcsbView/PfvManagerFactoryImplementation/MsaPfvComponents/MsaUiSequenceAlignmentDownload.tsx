...
export class MsaUiSequenceAlignmentDownload extends React.Component<MsaUiSequenceAlignmentDownloadInterface> {
  render() {
    return (
      <div
        title={"Download MSA of selected sequences"}
        onClick={() => this.click()}
  ...
  private async click(): Promise<void> {
    const targetAlignments = await this.props.rcsbFvContainer
      .get()
      ?.getAlignmentResponse();
    const targets = targetAlignments?.target_alignment?.map(
      (ta) => ta?.target_id,
    );
    if (!targets) return;
    const length = this.props.rcsbFvContainer.get()?.getFv().getBoardConfig()
      .length;
    if (!length) return;
    const msa: string[] = [];
    targetAlignments?.target_alignment?.forEach((ta) => {
      if (
        ta &&
        this.props.stateManager.assemblyModelSate
          .getMap()
          .has(ta.target_id ?? "none")
      ) {
        const sequence = ta.target_sequence;
        if (!sequence) return;
        if (!ta.aligned_regions) return;
        msa.push(`>${ta.target_id}|${ta.aligned_regions[0]?.query_begin}\n`);
        const sequenceAlignment: string[] = [];
        let prev =
          ta.aligned_regions[0]?.query_begin ?? Number.MIN_SAFE_INTEGER;
        if (prev > 1) sequenceAlignment.push("-".repeat(prev - 1));
        for (const ar of ta.aligned_regions ?? []) {
          ...
          sequenceAlignment.push(
            sequence.substring(ar.target_begin - 1, ar.target_end),
          );
          prev = ar.query_end;
        }
        if (prev < length) sequenceAlignment.push("-".repeat(length - prev));
