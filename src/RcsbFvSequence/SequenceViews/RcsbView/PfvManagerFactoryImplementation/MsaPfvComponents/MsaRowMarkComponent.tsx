...
export class MsaRowMarkComponent extends React.Component<
  MsaRowMarkInterface,
  MsaRowMarkState
> {
  ...
  public render(): JSX.Element {
    return (
      <>
        ...
        >
          <div
            className={classes.msaRowMark}
            onMouseOut={() => this.markHover(false)}
            onMouseOver={() => this.markHover(true)}
            style={{
              borderLeftColor: this.props.isGlowing
                ? this.state.markHoverColor
                : this.state.borderLeftColor,
            }}
          />
        </div>
      </>
  ...
  private markHover(flag: boolean): void {
    if (flag) this.setState({markHoverColor: this.ACTIVE_COLOR});
    else this.setState({markHoverColor: this.HOVER_COLOR});
  }
}
