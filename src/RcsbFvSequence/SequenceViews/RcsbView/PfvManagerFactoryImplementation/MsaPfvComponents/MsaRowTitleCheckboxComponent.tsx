...
type MsaRowTitleCheckboxType =
  | (MsaRowTitleCheckboxInterface & {entityId: string})
  | (MsaRowTitleCheckboxInterface & {instanceId: string});
...
export class MsaRowTitleCheckboxComponent extends React.Component<
  MsaRowTitleCheckboxType,
...
> {
  ...
  public async componentDidUpdate(
    prevProps: Readonly<MsaRowTitleCheckboxType>,
    prevState: Readonly<MsaRowTitleCheckboxState>,
    snapshot?: any,
  ) {
    if (!this.props.disabled && prevProps.disabled) this.requestInfo();
  }
  ...
  private subscribe(): void {
    this.subscription = this.props.stateManager.subscribe<
      ...
      } & {isComponent: boolean; isVisible: boolean}
    >((o) => {
  ...
  private structureViewerRepresentationChange(d: {
    label: string;
    isHidden: boolean;
  }): void {
  ...
  private style(): React.CSSProperties {
    ...
    return {
      ...
      backgroundColor: this.props.disabled
        ? color.disabled
        : color[this.state.checked ? "checked" : "unchecked"],
      border: 1,
      borderStyle: "solid",
  ...
  private getRcsbId(
    ...
    else return `${pdb.entryId}${TagDelimiter.instance}${pdb.instanceId}`;
  }
  ...
  private async componentInfo(data: {
    ...
    if (
      this.compId() == this.getRcsbId(data.pdb) &&
      this.props.tag == data.tag
    ) {
      if (data.isVisible)
        this.setState({checked: true, opacity: await this.opacity()});
      else this.setState({checked: false, opacity: await this.opacity()});
    }
  }
  ...
  private async polymerTest(): Promise<boolean> {
    ...
    const entryInfo = (
      await rcsbRequestCtxManager.getEntryProperties(entryId)
    )[0];
    const polymerChains = Array.from(
      entryInfo.entityToInstance.values(),
    ).flat();
    const assemblyChains = Array.from(
      entryInfo.instanceToOperator?.get(`${this.props.entryId}-1`)?.keys() ??
        [],
    ).filter((chId) => polymerChains.includes(chId));
    if (assemblyChains && assemblyChains.length > 1) return true;
    if (
      entryInfo &&
      (entryInfo.instanceToOperator
        ?.get(`${this.props.entryId}-1`)
        ?.get((entryInfo.entityToInstance.get(this.compId()) ?? [""])[0])
        ?.length ?? 0) > 1
    )
      return true;
    return false;
  ...
  private async nonPolymerTest(): Promise<boolean> {
    ...
    if (
      entryInfo &&
      Array.from(entryInfo.entityToPrd.values()).filter((v) => v != "").length >
        0
    )
      return true;
    if (
      entryInfo &&
      entryInfo.nonPolymerEntityToInstance &&
      entryInfo.nonPolymerEntityToInstance.size > 0
    )
      return true;
    return false;
  }
