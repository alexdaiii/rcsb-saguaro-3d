...
export class MsaRowTitleComponent extends React.Component<
  MsaRowTitleInterface,
  MsaRowTitleState
> {
  ...
  public render(): JSX.Element {
    return (
      ...
      >
        ...
        <div
          style={{cursor: this.cursor(), width: 39}}
          onClick={(e: MouseEvent) => this.altClick(e)}
        >
          <MsaRowTitleCheckboxComponent
            disabled={this.state.disabled}
            {...parseEntityOrInstance(this.props.targetAlignment.target_id!)}
            tag={"aligned"}
  ...
  private subscribe(): void {
    ...
    >((o) => {
      if (o.type == "model-change" && o.view == "1d-view") this.block();
      if (o.type == "model-change" && o.view == "3d-view") this.modelChange();
    });
  }
  ...
  private modelChange(): void {
    if (
      ...
      this.props.stateManager.assemblyModelSate
        .getMap()
        ?.has(this.props.targetAlignment.target_id)
    ) {
      if (this.state.disabled)
        this.setState({
          disabled: false,
          titleColor: this.ACTIVE_COLOR,
          blocked: false,
        });
