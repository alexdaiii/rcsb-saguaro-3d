...
> {
  ...
  render(): React.ReactElement {
    return (
      <input
        ...
        type={"checkbox"}
        checked={this.state.display === "visible"}
  ...
  private subscribe(): void {
    this.subscription = this.props.stateManager.subscribe<
      "component-info",
      ChainDisplayState & {label: string}
    >((o) => {
  ...
  private componentInfo(data: ChainDisplayState & {label: string}): void {
    if (data.label === this.props.label) this.setState({display: data.display});
  }
