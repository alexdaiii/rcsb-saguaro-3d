...
import {InterfaceInstanceTranslate} from "@rcsb/rcsb-saguaro-app/lib/RcsbUtils/Translators/InterfaceInstanceTranslate";
import {
  FeatureType,
...
interface AssemblyPfvManagerInterface
  extends PfvManagerFactoryConfigInterface<undefined> {
...
export class AssemblyPfvManagerFactory
  implements
    ...
    >
{
  public getPfvManager(
    config: AssemblyPfvManagerInterface,
  ): PfvManagerInterface {
...
class AssemblyPfvManager extends AbstractPfvManager<
  {instanceSequenceConfig?: InstanceSequenceConfig; useOperatorsFlag?: boolean},
  undefined
> {
  ...
  public async create(
  ...
  ): Promise<RcsbFvModulePublicInterface | undefined> {
    const onChangeCallback: Map<
      string,
      (x: PolymerEntityInstanceInterface) => void
    > = new Map<string, (x: PolymerEntityInstanceInterface) => {}>();
    const assemblyInstances: Map<string, Set<string>> = new Map<
      string,
      Set<string>
    >();
    this.stateManager.assemblyModelSate.forEach((v, k) => {
      assemblyInstances.set(
        v.entryId,
        new Set<string>(v.chains.map((d) => d.label)),
      );
      onChangeCallback.set(v.entryId, (x) => {
        this.stateManager.assemblyModelSate.set({
          entryId: v.entryId,
          labelAsymId: x.asymId,
          modelId: k,
        });
    ...
    const operatorNameContainer: DataContainer<string> =
      new DataContainer<string>(config.defaultOperatorName);
    if (this.stateManager.assemblyModelSate.get("entryId") != null) {
      this.module = await buildInstanceSequenceFv(
        ...
        this.stateManager.assemblyModelSate.getString("entryId"),
        {
          ...
          defaultValue:
            config.defaultAuthId ?? this.instanceSequenceConfig?.defaultValue,
          onChangeCallback: (context, module) => {
            onChangeCallback.get(
              this.stateManager.assemblyModelSate.getString("entryId"),
            )?.(context);
            const entryMap:
              | [
                  string,
                  {entryId: string; assemblyId: string; chains: ChainInfo[]},
                ]
              | undefined = Array.from(
              this.stateManager.assemblyModelSate.entries(),
            ).find((e) => e[1].entryId === context.entryId);
            const operator: OperatorInfo | undefined =
              entryMap && entryMap[0]
                ? getOperator(
                    this.stateManager.assemblyModelSate
                      .getMap()
                      .get(entryMap[0])!,
                    config.defaultAuthId,
                    operatorNameContainer.get(),
                  )
                : undefined;
          ...
          beforeChangeCallback: (x: PolymerEntityInstanceInterface) => {
            ...
            const entryMap:
              ...
              | undefined = Array.from(
              this.stateManager.assemblyModelSate.entries(),
            ).find((e) => e[1].entryId === x.entryId);
            if (!entryMap) {
              throw `Error: no modelId was found for ${x.entryId}`;
            }
            const operator: OperatorInfo | undefined = getOperator(
              this.stateManager.assemblyModelSate.getMap().get(entryMap[0])!,
              config.defaultAuthId,
              operatorNameContainer.get(),
            );
            this.addOperatorButton(operator?.name);
            this.stateManager.assemblyModelSate.setOperator(
              x.asymId,
              operator?.name,
            );
            operatorNameContainer.set(undefined);
            if (
              typeof this.additionalConfig?.operatorChangeCallback ===
                "function" &&
              this.stateManager.assemblyModelSate.getOperator()
            ) {
              this.additionalConfig.operatorChangeCallback(
                this.stateManager.assemblyModelSate.getOperator()!,
              );
            }
            if (
              (this.stateManager.assemblyModelSate.getChainInfo()?.operators
                ?.length ?? 0) > 1
            )
              return {
          ...
          filterInstances: assemblyInstances.get(
            this.stateManager.assemblyModelSate.getString("entryId"),
          ),
  ...
  private addOperatorButton(operatorName?: string): void {
    const currentChainInfo: ChainInfo | undefined =
      this.stateManager.assemblyModelSate.getChainInfo();
    if (
      this.useOperatorsFlag &&
      currentChainInfo &&
      currentChainInfo.operators.length > 1
    ) {
      ...
      RcsbFvUI.addSelectButton(
        ...
        currentChainInfo.operators.map((op) => ({
          ...
          onChange: async () => {
            ...
            await this.create({
              defaultAuthId:
                this.stateManager.assemblyModelSate.getChainInfo()?.auth,
  ...
  private filterFeatures(data: {
    ...
    return new Promise<Array<AnnotationFeatures>>(async (resolve) => {
      ...
      (
        await Promise.all(
          data.annotations.map(async (ann) => {
            if (
              ann.source == Source.PdbInterface &&
              ann.target_id &&
              data.rcsbContext?.asymId
            ) {
              const interfaceToInstance: InterfaceInstanceTranslate =
                await RcsbRequestContextManager.getInterfaceToInstance(
                  ann.target_id,
                );
              if (
                typeof ann.target_identifiers?.interface_partner_index ===
                  "number" &&
                ann.target_identifiers.assembly_id ===
                  this.stateManager.assemblyModelSate.getString("assemblyId") &&
                Array.isArray(interfaceToInstance.getOperatorIds(ann.target_id))
              ) {
                const operatorIds: string[][] | undefined =
                  interfaceToInstance.getOperatorIds(ann.target_id)?.[
                    ann.target_identifiers.interface_partner_index
                  ];
                if (
                  ann.features &&
                  this.stateManager.assemblyModelSate.getOperator() &&
                  ...
                  ann.features = ann.features.filter(
                    (f) => f && f.type == FeatureType.BurialFraction,
                  );
                  if (ann.features.length > 0) return ann;
                }
              }
        ...
        if (value) annotations = annotations.concat(value);
      });
      resolve(annotations);
    });
  }
...
function getOperator(
  entryInfo: {entryId: string; assemblyId: string; chains: Array<ChainInfo>},
  defaultAuthId?: string,
  defaultOperatorName?: string,
): OperatorInfo | undefined {
  const chainInfo: ChainInfo | undefined = defaultAuthId
    ? entryInfo.chains.find((ch) => ch.auth === defaultAuthId)
    : entryInfo.chains[0];
  if (chainInfo) {
    const operatorInfo: OperatorInfo | undefined = defaultOperatorName
      ? chainInfo.operators.find((op) => op.name === defaultOperatorName)
      : chainInfo.operators[0];
