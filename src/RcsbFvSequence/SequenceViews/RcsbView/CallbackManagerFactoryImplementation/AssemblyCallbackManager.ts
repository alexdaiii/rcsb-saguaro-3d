...
export class AssemblyCallbackManagerFactory
  implements CallbackManagerFactoryInterface<undefined>
{
  getCallbackManager(
    config: CallbackConfigInterface,
  ): CallbackManagerInterface<undefined> {
...
class AssemblyCallbackManager extends AbstractCallbackManager<undefined> {
    ...
    if (e == null) {
      ...
      if (e.isEmpty)
        ...
        this.stateManager.selectionState.setLastSelection({
          ...
          operatorName,
          source: "sequence",
          regions: processGaps(modelId, labelAsymId, e, operatorName).map(
            (r) => ({...r, source: "sequence"}),
          ),
        });
    }
  ...
  public highlightHoverCallback(
    selection: RcsbFvTrackDataElementInterface[],
  ): void {
    this.select(selection, "hover", "set");
  }

  public async modelChangeCallback(
    defaultAuthId?: string,
    defaultOperatorName?: string,
  ): Promise<void> {
    this.rcsbFvContainer.set(
      await this.pfvFactory.create({defaultAuthId, defaultOperatorName}),
    );
  }
  ...
  protected async innerStructureViewerSelectionChange(
    ...
    const lastSel: SaguaroRegionList | null =
    ...
    if (allSel == null || allSel.length === 0) {
      ...
      mode === "select" &&
      ((lastSel?.labelAsymId && lastSel?.labelAsymId != labelAsymId) ||
        (lastSel?.operatorName && lastSel?.operatorName != operatorName))
    ) {
      const labelAsymId: string | undefined =
        this.stateManager.assemblyModelSate.getChainInfo(lastSel?.labelAsymId!)
          ?.label;
      ...
      if (sel == null) {
        ...
        this.rcsbFvContainer
          .get()
          ?.getFv()
          .setSelection({elements: sel.regions, mode: mode});
      }
    }
  }

  protected innerPfvSelectionChange(
    selection: Array<RcsbFvTrackDataElementInterface>,
  ): void {
    if (
      selection.length == 0 &&
      this.stateManager.selectionState.getLastSelection() != null
    )
      return;
