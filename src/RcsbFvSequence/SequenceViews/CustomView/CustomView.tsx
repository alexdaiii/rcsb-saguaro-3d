...
export type CustomViewStateInterface<R, L> = Omit<
  Omit<CustomViewInterface<R, L>, "modelChangeCallback">,
  "structureViewer"
>;
...
export interface FeatureBlockInterface<R, L> {
  ...
  featureViewConfig:
    | Array<FeatureViewInterface<R, L>>
...
export class CustomView<R, L> extends AbstractView<
  CustomViewInterface<R, L> & {
    structureViewer: ViewerActionManagerInterface<R, L>;
  },
  CustomViewStateInterface<R, L>
> {
  private blockViewSelector: BlockSelectorManager = new BlockSelectorManager(
    this.blockChange.bind(this),
  );
  private boardMap: Map<string, FeatureViewInterface<R, L>> = new Map<
    string,
    FeatureViewInterface<R, L>
  >();
  private blockMap: Map<string, Array<string>> = new Map<
    string,
    Array<string>
  >();
  ...
  componentDidMount(): void {
    ...
    this.blockViewSelector.setActiveBlock(
      (this.state.blockConfig instanceof Array
        ? this.state.blockConfig
        : [this.state.blockConfig])[0].blockId!,
    );
  }
  ...
  componentDidUpdate(
    prevProps: Readonly<CustomViewInterface<R, L> & AbstractViewInterface>,
  ...
  private mapBlocks(
    config: FeatureBlockInterface<R, L> | Array<FeatureBlockInterface<R, L>>,
  ) {
    this.rcsbFvMap.forEach((pfv, id) => {
      pfv.unmount();
    });
    ...
    (config instanceof Array ? config : [config]).forEach((block) => {
      ...
      if (!this.blockMap.has(block.blockId))
        this.blockMap.set(block.blockId, new Array<string>());
      (block.featureViewConfig instanceof Array
        ? block.featureViewConfig
        : [block.featureViewConfig]
      ).forEach((board) => {
  ...
  private blockChange(): void {
    ...
    asyncScheduler.schedule(() => {
      if (typeof this.state.blockChangeCallback === "function")
        this.state.blockChangeCallback(
          this.props.structureViewer,
          Array.from(
            this.blockMap
              .get(this.blockViewSelector.getActiveBlock())!
              .values(),
          ).map((boardId) => this.rcsbFvMap.get(boardId)!),
          this.props.stateManager,
        );
      else this.structureSelectionCallback();
    }, 1000);
  }

  private unmountBlockFv() {
    this.blockMap
      .get(this.blockViewSelector.getPreviousBlock())
      ?.forEach((boardId) => {
        if (this.rcsbFvMap.get(boardId) == null) return;
  ...
  private buildBlockFv() {
    this.blockMap
      .get(this.blockViewSelector.getActiveBlock())
      ?.forEach((boardId) => {
        if (this.boardMap.get(boardId) == null) return;
        ...
        const width: number =
          window.document
            .getElementById(this.componentDivId)
            ?.getBoundingClientRect().width ?? 0;
        const trackWidth: number =
          width -
          (this.boardMap.get(boardId)!.boardConfig?.rowTitleWidth ?? 190) -
          55;
        const rcsbFv: RcsbFv = new RcsbFv({
          ...
          boardConfigData: {
            ...
            trackWidth: this.boardMap.get(boardId)!.boardConfig?.trackWidth
              ? this.boardMap.get(boardId)!.boardConfig?.trackWidth! - 4
              : trackWidth,
            selectionChangeCallback: (
              selection: RcsbFvTrackDataElementInterface[],
            ) => {
              if (this.innerSelectionFlag) return;
              this.boardMap
                .get(boardId)!
                .sequenceSelectionChangeCallback(
                  this.props.structureViewer,
            ...
            elementClickCallback: (
              element?: RcsbFvTrackDataElementInterface,
            ) => {
              this.boardMap
                .get(boardId)!
                .sequenceElementClickCallback(
  ...
  additionalContent(): JSX.Element {
    if (this.state.blockSelectorElement == null) return <></>;
  ...
  modelChangeCallback(): void {
    ...
    if (typeof this.props.modelChangeCallback === "function") {
      let newConfig: CustomViewStateInterface<R, L> =
        this.props.modelChangeCallback();
      if (newConfig != null) {
        this.updateContext = "state-change";
        if (
          newConfig.blockConfig != null &&
          newConfig.blockSelectorElement != null
        ) {
          this.mapBlocks(newConfig.blockConfig);
          this.setState({
            blockConfig: newConfig.blockConfig,
            blockSelectorElement: newConfig.blockSelectorElement,
          });
        } else if (
          newConfig.blockConfig == null &&
          newConfig.blockSelectorElement != null
  ...
  setState<K extends keyof CustomViewStateInterface<R, L>>(
    state:
      | ((
          ...
          | Pick<CustomViewStateInterface<R, L>, K>
          | CustomViewStateInterface<R, L>
          | null)
      | Pick<CustomViewStateInterface<R, L>, K>
      | CustomViewStateInterface<R, L>
  ...
  updateDimensions(): void {
    const div: HTMLElement | undefined | null = document.getElementById(
      this.componentDivId,
    )?.parentElement;
    const width: number =
      window.document
