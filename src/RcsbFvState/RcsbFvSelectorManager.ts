...
export class RcsbFvSelectorManager {
  private lastSelection:
    | (SaguaroRegionList & {source: RegionSelectionInterface["source"]})
    | null = null;
  ...
  public selectFromMultipleRegions(
    operation: "set" | "add",
    regions: {
      modelId: string;
      labelAsymId: string;
  ...
  public addSelectionFromMultipleRegions(
    regions: (SaguaroChain & {region: RegionSelectionInterface})[],
    mode: "select" | "hover",
  ): void {
    regions.forEach((r) => {
      this.addSelectionFromRegion(
  ...
  public setSelectionFromResidueSelection(
  ...
  public getSelection(mode: "select" | "hover"): Array<SaguaroRegionList> {
    if (mode === "select") return this.selection;
    else return this.hover;
  }
  ...
  public getSelectionWithCondition(
    modelId: string,
    labelAsymId: string,
    mode: "select" | "hover",
    operatorName?: string,
  ): SaguaroRegionList | undefined {
    const sel: Array<SaguaroRegionList> =
      mode === "select"
        ? this.selection.filter(
            (d) =>
              d.modelId === modelId &&
              d.labelAsymId === labelAsymId &&
              (d.operatorName === operatorName || !operatorName),
          )
        : this.hover.filter(
            (d) =>
              d.modelId === modelId &&
    ...
    if (sel.length > 0) {
      return {
        ...
        regions: ([] as RegionSelectionInterface[]).concat.apply(
          [],
          sel.map((s) => s.regions),
        ),
      };
  ...
  public clearSelection(
    mode: "select" | "hover",
    selection?: Partial<SaguaroChain>,
  ): void {
    if (!selection) {
      if (mode === "select") this.selection = new Array<SaguaroRegionList>();
      else this.hover = new Array<SaguaroRegionList>();
    } else if (selection.labelAsymId || selection.modelId) {
      if (mode === "select") {
        this.selection = this.selection.filter((r) =>
          selectionFilter(r, selection),
        );
      } else {
  ...
  public selectionSource(
    mode: "select" | "hover",
    region: SaguaroChain & SaguaroRange,
  ): "structure" | "sequence" | undefined {
    if (mode === "select") {
      return this.selection
        .filter(
          (r) =>
            r.modelId === region.modelId &&
            r.labelAsymId === region.labelAsymId,
        )[0]
        ?.regions.filter(
          (r) => r.begin === region.begin && r.end === region.end,
        )[0]?.source;
    } else {
      return this.hover
        .filter(
...
function selectionFromResidueSelection(
  res: Array<SaguaroSet>,
  mode: "select" | "hover",
  source: "structure" | "sequence",
): Array<SaguaroRegionList> {
  const none: "none" = "none";
  const selMap: Map<string, Map<string, Map<string, Set<number>>>> = new Map<
    string,
  ...
  res.forEach((r) => {
    ...
    if (
      r.operatorName &&
      !selMap.get(r.modelId)!.get(r.labelAsymId)!.has(r.operatorName)
    ) {
      selMap
        .get(r.modelId)!
        .get(r.labelAsymId)!
        .set(r.operatorName, new Set<number>());
    } else {
...
function buildIntervals(
  ids: Set<number>,
  source: "structure" | "sequence",
): Array<RegionSelectionInterface> {
  const out: Array<RegionSelectionInterface> =
    new Array<RegionSelectionInterface>();
...
function selectionFilter(
  ...
  return (
    (typeof selection.modelId === "string" && r.modelId != selection.modelId) ||
    (typeof selection.labelAsymId === "string" &&
      r.labelAsymId != selection.labelAsymId) ||
    (typeof selection.operatorName === "string" &&
      r.operatorName != selection.operatorName)
  );
}
