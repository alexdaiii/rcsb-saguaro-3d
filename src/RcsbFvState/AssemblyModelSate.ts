...
export class AssemblyModelSate {
  ...
  constructor(modelMap?: SaguaroPluginModelMapType) {
    if (modelMap) this.modelMap = modelMap;
  }
  ...
  public get(
    key: keyof AssemblyModelStateInterface,
  ): string | OperatorInfo | undefined {
    return this.state[key];
  }

  public getString(
    key: keyof Omit<AssemblyModelStateInterface, "operator">,
  ): string {
    if (!this.state[key]) throw `${key} is undefined`;
    return this.state[key]!;
  }
  ...
  public forEach(
    f: (
      v: {entryId: string; assemblyId: string; chains: Array<ChainInfo>},
      k: string,
    ) => void,
  ): void {
  ...
  public entries(): IterableIterator<
    [string, {entryId: string; assemblyId: string; chains: Array<ChainInfo>}]
  > {
    return this.modelMap.entries();
  }

  public setOperator(asymId?: string, opName?: string) {
    const currentChainInfo: ChainInfo | undefined = this.getChainInfo(
      asymId ?? this.state.labelAsymId,
    );
    this.state.operator = opName
      ? currentChainInfo?.operators.filter((op) => op.name === opName)[0]
      : currentChainInfo?.operators[0];
  }

  public getModelChainInfo(
    modelId: string,
  ):
    | {entryId: string; assemblyId: string; chains: Array<ChainInfo>}
    | undefined {
    return this.modelMap?.get(modelId);
  }

  public getChainInfo(asymId?: string): ChainInfo | undefined {
    if (!this.state.modelId) throw "modelId not define";
    if (asymId)
      return this.modelMap
        .get(this.state.modelId)
        ?.chains.find((ch) => ch.label === asymId);
    else
      return this.modelMap
        .get(this.state.modelId)
        ?.chains.find((ch) => ch.label === this.state.labelAsymId);
  }
