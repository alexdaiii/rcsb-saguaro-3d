...
type StructureObject = StateObjectSelector<
...
export const AssemblyTrajectoryPresetProvider =
  TrajectoryHierarchyPresetProvider({
    ...
    ): ParamDefinition.For<AssemblyTrajectoryParamsType> => ({
    ...
    async apply(trajectory, params, plugin) {
      ...
      if (params.asymId) {
        ...
        do {
          ...
          structure = await builder.createStructure(modelProperties, {
            name: "assembly",
          ...
          if (cell) {
            ...
            const strData: Structure = (cell.obj as StateObject<Structure>)
              .data;
            if (units) {
      ...
      const structureProperties: StructureObject =
        await builder.insertStructureProperties(structure);
      if (
        !plugin.representation.structure.themes.colorThemeRegistry.has(
      ...
      const unitcell: StateObjectSelector | undefined =
        await builder.tryCreateUnitcell(modelProperties, undefined, {
          isHidden: true,
        });
      const representation:
        | StructureRepresentationPresetProvider.Result
        | undefined =
