    ...
    return Task.create("Build Flexible Chain", async (ctx) => {
      ...
      for (const unit of structure.units) {
        ...
        if (
          ...
          if (alignedType != "polymer") throw new Error("");
          ...
          for (const trans of params.transform ?? []) {
            ...
            const ts: Structure = Structure.transform(
              s,
              trans.transform as unknown as Mat4,
            );
            const u = ts.units[0];
            builder.addUnit(
              u.kind,
          ...
          return new PSO.Molecule.Structure(blockStructure, {
