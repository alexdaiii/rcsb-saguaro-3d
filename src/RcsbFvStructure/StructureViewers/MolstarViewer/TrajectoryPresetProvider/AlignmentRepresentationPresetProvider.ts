...
type ComponentType = Awaited<
  ReturnType<
    InstanceType<typeof StructureBuilder>["tryCreateComponentFromExpression"]
  >
>;
type RepresentationType = ReturnType<
...
export const AlignmentRepresentationPresetProvider =
  StructureRepresentationPresetProvider({
    ...
    isApplicable: (
      structureRef: PluginStateObject.Molecule.Structure,
      plugin: PluginContext,
    ): boolean => true,
    params: (
      structureRef: PluginStateObject.Molecule.Structure | undefined,
      plugin: PluginContext,
    ) => ({
      pdb: PD.Value<
        | {entryId: string; entityId: string}
    ...
    apply: async (
      structureRef: StateObjectRef<PluginStateObject.Molecule.Structure>,
      params: RepresentationParamsType,
      plugin: PluginContext,
    ) => {
      ...
      const entityId =
        params.pdb && "entityId" in params.pdb
          ? params.pdb?.entityId
          : undefined;
      const instanceId =
        params.pdb && "instanceId" in params.pdb
      ...
      for (const unit of structure.units) {
        ...
        if (
          SP.chain.label_entity_id(l) == entityId ||
          SP.chain.label_asym_id(l) == instanceId
        ) {
          ...
          const alignedOperators: string[] =
            SP.unit.pdbx_struct_oper_list_ids(l);
          if (alignedOperators.length == 0) alignedOperators.push("0");
          if (alignedType != "polymer") return {};
          if (
          ...
          const comp =
            await plugin.builders.structure.tryCreateComponentFromExpression(
              ...
              uniqid(
                `${entryId}${TagDelimiter.entity}${alignedEntityId}${
                  TagDelimiter.instance
                }${alignedAsymId}${TagDelimiter.entity}${alignedOperators.join(
                  ",",
                )}`,
              ),
              {
                label: `${entryId}${TagDelimiter.entity}${alignedEntityId}${
                  TagDelimiter.instance
                }${alignedAsymId}${
                  TagDelimiter.assembly
                }${alignedOperators.join(",")}${
                  TagDelimiter.assembly
                }${alignedType}`,
              },
            );
          ...
          representationMap["aligned"] = builder.buildRepresentation(
            update,
            comp,
            {
              color: PLDDTConfidenceColorThemeProvider.isApplicable({structure})
                ? (PLDDTConfidenceColorThemeProvider.name as ColorTheme.BuiltIn)
                : "chain-id",
              type: "cartoon",
            },
          );
      ...
      representationMap["polymer"] = builder.buildRepresentation(
        update,
        comp,
        {
          color: PLDDTConfidenceColorThemeProvider.isApplicable({structure})
      ...
      for (const expression of createSelectionExpressions(entryId)) {
        if (expression.tag == "polymer") continue;
        const comp =
          await plugin.builders.structure.tryCreateComponentFromExpression(
            structureCell,
            expression.expression,
            uniqid(
              `${entryId}${TagDelimiter.entity}${alignedEntityId}${TagDelimiter.assembly}${expression.tag}`,
            ),
            {
        ...
        representationMap[expression.tag] = builder.buildRepresentation(
          update,
          comp,
          {
        ...
        if (expression.type !== "ball-and-stick")
          representationMap[expression.tag + "#ball-and-stick"] =
      ...
      structure.inheritedPropertyData.reprList = Object.values(
        representationMap,
      ).filter((repr) => typeof repr != "undefined");
      await updateFocusRepr(plugin, structure, FOCUS_RESIDUE_COLOR, {});
...
async function structuralAlignment(
  plugin: PluginContext,
  ref: StructureAlignmentParamsType,
  pdb: StructureAlignmentParamsType,
  structure: Structure,
): Promise<void> {
  if (ref.entryId == pdb.entryId) {
    ...
    const pdbUnit: {unit: Unit; localScore: Map<number, number>} | undefined =
      await findFirstInstanceUnit(pdbData, pdb.labelAsymId);
    const refUnit: {unit: Unit; localScore: Map<number, number>} | undefined =
      refData
        ? await findFirstInstanceUnit(refData, ref.labelAsymId)
        : undefined;
    if (
      pdbUnit &&
      refUnit &&
      ref.targetAlignment?.aligned_regions &&
      pdb.targetAlignment?.aligned_regions
    ) {
      const alignmentList = AM.getAllTargetIntersections(
        ref.targetAlignment.aligned_regions as AlignedRegion[],
        pdb.targetAlignment.aligned_regions as AlignedRegion[],
      );
      alignmentList.forEach((alignment) => {
        const refRange = AM.range(
          alignment[0].target_begin,
          alignment[0].target_end,
        );
        const pdbRange = AM.range(
          alignment[1].target_begin,
          alignment[1].target_end,
        );
        refRange.forEach((refIndex, n) => {
          ...
          if (
            !Loci.isEmpty(pdbLoci) &&
            !Loci.isEmpty(refLoci) &&
            checkLocalScore(pdbUnit.localScore, pdbIndex) &&
            checkLocalScore(refUnit.localScore, refIndex)
          ) {
    ...
    if (pdbData && pdbUnit && refData && refUnit) {
      const refLoci: Loci = residueListToLoci(
        refParams!,
        refResIndexes,
        refData,
      );
      ...
      if (
        StructureElement.Loci.is(refLoci) &&
        StructureElement.Loci.is(pdbLoci)
      ) {
        const pivot = plugin.managers.structure.hierarchy.findStructure(
          refLoci.structure,
        );
        const coordinateSystem =
          pivot?.transform?.cell.obj?.data.coordinateSystem;
        const transforms = superpose([refLoci, pdbLoci]);
        const {bTransform} = transforms[0];
        await transform(
          plugin,
          plugin.helpers.substructureParent.get(pdbData)!,
          bTransform,
          coordinateSystem,
        );
      }
...
async function findFirstInstanceUnit(
  structure: Structure,
  labelAsymId: string,
): Promise<{unit: Unit; localScore: Map<number, number>} | undefined> {
  const l = StructureElement.Location.create(structure);
  for (const unit of structure.units) {
    StructureElement.Location.set(l, structure, unit, unit.elements[0]);
...
function checkLocalScore(
  ...
  if (scoreMap.size == 0) return true;
  return !!(scoreMap.get(index) && scoreMap.get(index as ResidueIndex)! >= 70);
}

async function obtainQualityAssessment(
  model: Model,
...
async function transform(
  ...
  const o = plugin.state.data.selectQ((q) =>
    q
      ...
      .subtree()
      .withTransformer(TransformStructureConformation),
  )[0];
  ...
  const b = o
    ...
    : plugin.state.data
        .build()
        .to(s)
        .insert(TransformStructureConformation, params, {
          tags: SuperpositionTag,
        });
  await plugin.runTask(plugin.state.data.updateTree(b));
}

function residueToLoci(
  pdb: StructureAlignmentParamsType,
  pdbIndex: number,
  structure: Structure,
): Loci {
  const expression = MS.struct.generator.atomGroups({
    ...
    "atom-test": MS.core.logic.and([
      ...
      MS.core.logic.or([
        MS.core.rel.eq([MS.ammp("label_alt_id"), ""]),
        MS.core.rel.eq([MS.ammp("label_alt_id"), "A"]),
      ]),
    ]),
  });
...
function residueListToLoci(
  ...
  const expression = MS.struct.generator.atomGroups({
    ...
    "residue-test": MS.core.logic.or(
      indexList.map((index) =>
        MS.core.rel.eq([MS.ammp("label_seq_id"), index]),
      ),
    ),
    "atom-test": MS.core.logic.and([
      MS.core.rel.eq([MS.ammp("label_atom_id"), "CA"]),
