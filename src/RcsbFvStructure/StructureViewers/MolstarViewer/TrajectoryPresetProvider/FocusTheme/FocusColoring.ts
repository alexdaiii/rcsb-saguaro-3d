...
function FocusResidueColorTheme(
  ctx: ThemeDataContext,
  props: ParamDefinition.Values<ElementSymbolColorThemeParams>,
  ...
  const bondColor = (location: StructureElement.Location) => {
    for (const repr of ctx.structure?.inheritedPropertyData.reprList) {
      const layer: Layer = repr.obj.data.repr.state.overpaint.layers.find(
        (layer: Layer) => {
          if (!StructureElement.Loci.is(layer.loci)) return false;
          return isLocationInLoci(location, layer.loci);
        },
      );
      if (layer) return layer.color;
    }
    return ctx.structure?.inheritedPropertyData.reprList[0].obj.data.repr.theme.color.color(
      location,
    );
  };

  const nonCarbonAtomsColor = ElementSymbolColorTheme(ctx, props).color as (
    location: Location,
  ) => Color;
  const atomColor = (location: StructureElement.Location) => {
    if (
      location.unit.model.atomicHierarchy.atoms.type_symbol.value(
        location.element,
      ) == "C"
    )
...
function isLocationInLoci(
  ...
  const loc: StructureElement.Location = StructureElement.Location.create(
  ...
  for (let n = 0; n < OrderedSet.size(currentLoci.elements[0].indices); n++) {
    StructureElement.Location.set(
      ...
      currentLoci.elements[0].unit.elements[
        OrderedSet.getAt(currentLoci.elements[0].indices, n)
      ],
    );
