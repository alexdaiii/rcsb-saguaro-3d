...
export class MolstarModelMapManager<L>
  implements
    ViewerModelMapManagerInterface<
      {loadMethod: LoadMethod; loadParams: LoadParams},
      L
    >
{
  private readonly viewer: Viewer;
  private readonly modelMap: Map<string, string | undefined> = new Map<
    string,
    string
  >();
  ...
  constructor(
    viewer: Viewer,
    delegateModelIdFromTrajectory: (trajectory: L) => string | undefined,
  ) {
    this.viewer = viewer;
  ...
  public add(
    lC: {loadMethod: LoadMethod; loadParams: LoadParams},
    trajectory: L,
  ) {
  ...
  public getChains(): SaguaroPluginModelMapType {
    ...
    const out: SaguaroPluginModelMapType = new Map<
      string,
      {entryId: string; chains: Array<ChainInfo>; assemblyId: string}
    >();
    structureRefList.forEach((structureRef, i) => {
      const structure: Structure = getStructure(
        structureRef[0],
        this.viewer.plugin.state.data,
      );
      let modelEntityId = getModelEntityOptions(structure)[0][0];
      const chains: [
        {modelId: string; entryId: string; assemblyId: string},
        ChainInfo[],
      ] = getChainValues(structure, modelEntityId);
      if (chains.length > 0 && chains[0].modelId)
        out.set(this.getModelId(chains[0].modelId), {
          entryId: chains[0].entryId,
          assemblyId: chains[0].assemblyId,
          chains: chains[1],
        });
    });
    return out;
  ...
  private map(loadParams: LoadParams, trajectory: L): void {
    ...
    if (!modelId || !loadParams.id) throw new Error("modelId not found");
...
): [{modelId: string; entryId: string; assemblyId: string}, ChainInfo[]] {
  ...
  for (const unit of structure.units) {
    ...
    if (chains.has(chId)) {
      ...
      chains.set(chId, {
        ...
        auth: SP.chain.auth_asym_id(l),
        entityId: SP.entity.id(l),
        title: SP.entity.pdbx_description(l).join("|"),
        type: SP.entity.type(l),
        operators: [opKey(l)],
      });
    }
  }
  const id: {modelId: string; entryId: string; assemblyId: string} = {
    modelId: l.unit?.model?.id,
    entryId: l.unit?.model?.entryId,
