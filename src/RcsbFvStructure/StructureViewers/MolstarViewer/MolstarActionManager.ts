...
export class MolstarActionManager<P, L>
  implements ViewerActionManagerInterface<LoadMolstarInterface<P, L>, L>
{
  ...
  private readonly modelMapManager: ViewerModelMapManagerInterface<
  ...
  async load(
    loadConfig: LoadMolstarInterface<P, L>[],
  ): Promise<(L | undefined)[]>;
  async load(
    loadConfig: LoadMolstarInterface<P, L> | LoadMolstarInterface<P, L>[],
    ...
    for (const lC of Array.isArray(loadConfig) ? loadConfig : [loadConfig]) {
      if (checkLoadData(lC)) {
        if (lC.loadMethod == LoadMethod.loadPdbId) {
          ...
          out.push(
            (await this.viewer.loadPdbId(config.entryId!, {
              props: config.props,
              matrix: config.matrix,
              reprProvider: config.reprProvider,
          ...
          const config: LoadParams<P, L> = lC.loadParams as LoadParams<P, L>;
          out.push(
            (await this.viewer.loadStructureFromData(
              config.data!,
              config.format!,
              config.isBinary!,
              {
        ...
        if (trajectory) this.modelMapManager.add(lC, trajectory);
      }
    }
  ...
  async removeStructure(
    loadConfig: LoadMolstarInterface<P, L> | Array<LoadMolstarInterface<P, L>>,
  ): Promise<void> {
    loadConfig = Array.isArray(loadConfig) ? loadConfig : [loadConfig];
    loadConfig.forEach((lC) => {
      (Array.isArray(lC.loadParams) ? lC.loadParams : [lC.loadParams]).forEach(
        (loadParams) => {
          if (typeof loadParams.id === "string") {
            const pdbStr: StructureRef | undefined =
              this.viewer.plugin.managers.structure.hierarchy.current.structures.find(
                (s) =>
                  s.properties?.cell?.obj?.data?.units[0]?.model?.id ==
                  this.modelMapManager.getModelId(loadParams.id!),
              );
            if (pdbStr) {
  ...
  public select(...args: any[]): void {
    if (args[5] != undefined) {
      ...
      args[0].length > 0 &&
      typeof args[0][0].position === "number"
    ) {
      this.selectSet(args[0], args[1], args[2]);
    } else if (
      Array.isArray(args[0]) &&
      args[0].length > 0 &&
      typeof args[0][0].begin === "number"
    ) {
    ...
    this.viewer.select(
      {
        ...
        labelSeqRange: {beg: begin, end: end},
  ...
  private selectSet(
    ...
    this.viewer.select(
      selection.map((r) => ({
        modelId: this.modelMapManager.getModelId(r.modelId),
        labelSeqId: r.position,
        labelAsymId: r.labelAsymId,
  ...
  private selectMultipleRanges(
    ...
    this.viewer.select(
      selection.map((r) => ({
        modelId: this.modelMapManager.getModelId(r.modelId),
        labelAsymId: r.labelAsymId,
        labelSeqRange: {beg: r.begin, end: r.end},
        operatorName: r.operatorName,
      })),
      mode,
      operation,
  ...
  public clearSelection(mode: "select" | "hover", option?: SaguaroChain): void {
    ...
    if (option != null)
      this.viewer.clearSelection(mode, {
        ...option,
        modelId: this.modelMapManager.getModelId(option.modelId),
      });
    else this.viewer.clearSelection(mode);
    this.innerSelectionFlag.set(false);
  }
  ...
  private focusPositions(
    ...
    const structure: Structure | undefined = getStructureWithModelId(
      this.viewer.plugin.managers.structure.hierarchy.current.structures,
      this.modelMapManager.getModelId(modelId),
    );
    if (structure == null) return;
    const chainTests: Expression[] = [
      MS.core.rel.eq([MS.ammp("label_asym_id"), labelAsymId]),
    ];
    if (operatorName)
      chainTests.push(MS.core.rel.eq([operatorName, MS.acp("operatorName")]));
    const sel: StructureSelection = Script.getStructureSelection(
      (Q) =>
        Q.struct.generator.atomGroups({
          "chain-test": Q.core.logic.and(chainTests),
          "residue-test": Q.core.set.has([
            MS.set(...SetUtils.toArray(new Set(positions))),
    ...
    if (Array.isArray(args[1])) {
      if (args[1].length > 0) {
        if (typeof args[1][0].position === "number") {
          await this.viewer.createComponent(
            args[0],
            args[1].map((r) => ({
              modelId: this.modelMapManager.getModelId(r.modelId),
              labelAsymId: r.labelAsymId,
      ...
      await this.viewer.createComponent(
        args[0],
        {
          modelId: this.modelMapManager.getModelId(args[1]),
          labelAsymId: args[2],
          labelSeqRange: {beg: args[3], end: args[4]},
          operatorName: args[6],
        },
        args[5],
      );
  ...
  public isComponent(componentLabel: string): boolean {
    for (const c of this.viewer.plugin.managers.structure.hierarchy
  ...
  public async colorComponent(
    ...
    for (const c of this.viewer.plugin.managers.structure.hierarchy
      ...
      for (const comp of c) {
        if (comp.cell.obj?.label === componentLabel) {
          await this.viewer.plugin.managers.structure.component.updateRepresentationsTheme(
            [comp],
            {color: color},
          );
          return;
  ...
  public getComponentSet(): Set<string> {
    ...
    this.viewer.plugin.managers.structure.hierarchy.currentComponentGroups.forEach(
      (c) => {
        for (const comp of c) {
  ...
  public async removeComponent(componentLabel?: string): Promise<void> {
    if (componentLabel == null) {
      this.viewer.plugin.managers.structure.hierarchy.currentComponentGroups.forEach(
        (c) => this.viewer.plugin.managers.structure.hierarchy.remove(c),
      );
    } else {
  ...
  public displayComponent(
    ...
    else return this.getComponentDisplay(componentLabel);
  }
...
function getStructureWithModelId(
