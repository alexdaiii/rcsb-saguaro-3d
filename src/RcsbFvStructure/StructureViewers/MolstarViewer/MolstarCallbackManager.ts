...
type ModelMapType = Omit<
  ViewerModelMapManagerInterface<unknown, unknown>,
  "add" | "delete"
>;
export class MolstarCallbackManager implements ViewerCallbackManagerInterface {
  ...
  public subscribeRepresentationChange(): Subscription {
    this.reprChangeSubs =
      this.viewer.plugin.state.data.events.cell.stateUpdated.subscribe((s) => {
        if (this.innerReprChangeFlag.get()) return;
        if (
          s.cell.obj?.tags?.find((t) => t.indexOf("structure-component-") === 0)
        )
          ...
          >({
            ...
            view: "3d-view",
            data: {label: s.cell.obj?.label, isHidden: !!s.cell.state.isHidden},
          });
      });
  ...
  public subscribeHover(): Subscription {
    this.hoverSubs = this.viewer.plugin.behaviors.interaction.hover.subscribe(
      (r) => {
        ...
        this.stateManager.selectionState.setSelectionFromResidueSelection(
          sequenceData,
          "hover",
          "structure",
        );
  ...
  public subscribeSelection(): Subscription {
    this.selectSubs =
      this.viewer.plugin.managers.structure.selection.events.changed.subscribe(
        () => {
          if (this.innerSelectionFlag.get()) return;
          if (
            this.viewer.plugin.managers.structure.selection.additionsHistory
              .length > 0
          ) {
            const currentLoci: Loci =
              this.viewer.plugin.managers.structure.selection
                .additionsHistory[0].loci;
            const loc: StructureElement.Location =
              StructureElement.Location.create(currentLoci.structure);
            StructureElement.Location.set(
              loc,
              currentLoci.structure,
              currentLoci.elements[0].unit,
              currentLoci.elements[0].unit.elements[
                OrderedSet.getAt(currentLoci.elements[0].indices, 0)
              ],
            );
            const currentModelId: string = this.modelMapManager.getModelId(
              currentLoci.structure.model.id,
            );
              ...
              if (SP.entity.type(loc) === "non-polymer") {
                ...
                const query: StructureQuery = Q.modifiers.includeSurroundings(
                  Q.generators.residues({
                    residueTest: (l) =>
                      SP.residue.auth_seq_id(l.element) === resAuthId,
                    chainTest: (l) =>
                      SP.chain.label_asym_id(l.element) === chainLabelId,
                  }),
                  {
                    radius: 5,
                    wholeResidues: true,
                ...
                const sel: StructureSelection = StructureQuery.run(
                  query,
                  currentLoci.structure,
                );
                const surroundingsLoci: Loci =
                  StructureSelection.toLociWithSourceUnits(sel);
                this.viewer.plugin.managers.structure.selection.fromLoci(
                  "add",
                  surroundingsLoci,
                );
                const surroundingsLoc = StructureElement.Location.create(
                  surroundingsLoci.structure,
                );
            ...
            else this.stateManager.selectionState.setLastSelection(null);
          } else {
            this.stateManager.selectionState.setLastSelection(null);
          }
          const sequenceData: Array<SaguaroSet> = new Array<SaguaroSet>();
          for (const structure of this.viewer.plugin.managers.structure
            .hierarchy.current.structures) {
            const data: Structure | undefined = structure.cell.obj?.data;
            if (data == null) return;
            const loci: Loci =
              this.viewer.plugin.managers.structure.selection.getLoci(data);
            if (StructureElement.Loci.is(loci)) {
              const loc = StructureElement.Location.create(loci.structure);
              for (const e of loci.elements) {
                StructureElement.Location.set(
                ...
                if (SP.entity.type(loc) === "polymer") {
                  const seqIds = new Set<number>();
                  for (
                    let i = 0, il = OrderedSet.size(e.indices);
                    i < il;
                    ++i
                  ) {
                    loc.element =
                      e.unit.elements[OrderedSet.getAt(e.indices, i)];
                    seqIds.add(SP.residue.label_seq_id(loc));
                  }
                  if (seqIds.size > 0)
                    sequenceData.push({
  ...
  public subscribeModelChange(): Subscription {
    this.modelChangeSubs =
      this.viewer.plugin.state.behaviors.events.object.updated.subscribe(
        (o) => {
          if (this.loadingFlag.get()) return;
          this.modelChange();
        },
      );
  ...
  public modelChange(): void {
    this.stateManager.assemblyModelSate.setMap(
      this.modelMapManager.getChains(),
    );
