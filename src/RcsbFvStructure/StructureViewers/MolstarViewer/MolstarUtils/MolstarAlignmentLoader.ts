...
export class MolstarAlignmentLoader
  implements
    StructureLoaderInterface<
      [
    ...
    >
{
  private readonly loadParamsProvider?: LoadParamsProviderInterface<
    {entryId: string; instanceId: string},
  ...
  async load(
    ...
    const structureId: string = `${pdb.entryId}${
      "entityId" in pdb ? TagDelimiter.entity : TagDelimiter.instance
    }${"entityId" in pdb ? pdb.entityId : pdb.instanceId}`;
    if (!this.structureMap.has(structureId)) {
      this.structureMap.add(structureId);
      const loadParams =
        "instanceId" in pdb ? this.loadParamsProvider?.get(pdb) : undefined;
      return loadParams
        ? await structureViewer.load(loadParams)
