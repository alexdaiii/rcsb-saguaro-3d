...
export class StructureViewer<R, L, S extends {}>
  implements StructureViewerInterface<R, L, S>
{
  private readonly structureViewerManagerFactory: ViewerManagerFactoryInterface<
    R,
    L,
    S
  >;
  ...
  public init(stateManager: RcsbFvStateInterface, args: S): void {
    const {actionManager, callbackManager} =
      this.structureViewerManagerFactory.getViewerManagerFactory(
        stateManager,
        args,
      );
  ...
  async load(
    ...
    const out = await this.actionManager.load(
      Array.isArray(loadConfig) ? loadConfig : [loadConfig],
    );
    this.modelChange();
    return out;
  }
  ...
  public async createComponent(...args: any[]): Promise<void> {
    await this.actionManager.createComponent(
      args[0],
      args[1],
      args[2],
      args[3],
  ...
  public displayComponent(
    ...
    return this.actionManager.displayComponent(
      componentLabel as any,
      visibilityFlag as any,
    );
  }
