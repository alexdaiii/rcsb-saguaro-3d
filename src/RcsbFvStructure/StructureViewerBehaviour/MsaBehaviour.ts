...
export class MsaBehaviourObserver<R, L>
  implements StructureViewerBehaviourObserverInterface<R, L>
{
  ...
  public observe(
    ...
    this.structureBehaviour = new MsaBehaviour(
      structureViewer,
      stateManager,
...
class MsaBehaviour<R, L> implements StructureViewerBehaviourInterface {
  private readonly structureViewer: ViewerCallbackManagerInterface &
    ViewerActionManagerInterface<R, L>;
  ...
  private subscribe(): Subscription {
    ...
    >(async (o) => {
      ...
      if (o.type == "hover-change" && o.view == "1d-view") this.hoverChange();
  ...
  async featureClick(data?: SelectedRegion[]): Promise<void> {
    const cameraFocus = onetimeCall<SelectedRegion>((d: SelectedRegion) => {
      ...
      const residues: number[] = regions
        .map((r) => (r.begin == r.end ? [r.begin] : [r.begin, r.end]))
        .flat()
        .filter((r) => r != null);
      this.structureViewer.cameraFocus(
        modelId,
        labelAsymId,
        residues,
        operatorName,
    ...
    if (!data || data.length == 0) this.resetPluginView();
    const numRes = data
      ?.map((d) => d.region.end - d.region.begin + 1)
    ...
    data?.forEach((d) => {
      ...
      if (
        ...
        if (
          data
            ?.map((d) =>
              d.region.end - d.region.begin + 1 < this.CREATE_COMPONENT_THR
                ? 1
                : 0,
            )
          ...
          asyncScheduler.schedule(async () => {
            ...
            const selectedComponentId = `${modelId}${TagDelimiter.instance}${
              labelAsymId +
              ":" +
              (x === y ? x.toString() : x.toString() + "," + y.toString())
            }`;
            await this.structureViewer.createComponent(
              selectedComponentId!,
              ranges,
              "ball-and-stick",
            );
  ...
  reprChange(
    data?: {
      pdb:
        | {entryId: string; entityId: string}
        | {entryId: string; instanceId: string};
    } & {tag: "aligned" | "polymer" | "non-polymer"; isHidden: boolean},
  ): void {
    if (data) {
      const chainInfo: ChainInfo | undefined =
        this.stateManager.assemblyModelSate
          .getModelChainInfo(this.getRcsbId(data.pdb))
          ?.chains.find(
            (ch) =>
              ("entityId" in data.pdb && ch.entityId == data.pdb.entityId) ||
              ("instanceId" in data.pdb && ch.label == data.pdb.instanceId),
          );
      if (!chainInfo) return;
      switch (data.tag) {
        case "aligned":
          ...
          const alignedCompId: string = `${data.pdb.entryId}${
            TagDelimiter.entity
          }${chainInfo.entityId}${TagDelimiter.instance}${asymId}${
            TagDelimiter.assembly
          }${operatorInfo[0].ids.join(",")}${
            TagDelimiter.assembly
          }${"polymer"}`;
        ...
        case "polymer":
          ...
          break;
        case "non-polymer":
          createSelectionExpressions(data.pdb.entryId)
            .map((expression) => expression.tag)
            .filter((tag) => tag != "water" && tag != "polymer")
            .forEach((tag) => {
              const nonPolymerCompId: string = `${data.pdb.entryId}${TagDelimiter.entity}${chainInfo.entityId}${TagDelimiter.assembly}${tag}`;
              this.structureViewer.displayComponent(
                nonPolymerCompId,
                !data.isHidden,
              );
            });
  ...
  async modelChange(data?: AlignmentDataType): Promise<void> {
    if (data) {
      await this.structureLoader.load(
        this.structureViewer,
        data.pdb,
  ...
  private select(mode: "select" | "hover"): void {
    if (mode == "select") this.structureViewer.clearFocus();
    if (this.stateManager.selectionState.getSelection(mode).length == 0)
      this.structureViewer.clearSelection(mode);
    this.structureViewer.select(
      this.stateManager.selectionState
        .getSelection(mode)
        .map((selectedRegion) => {
  ...
  private async removeComponent(): Promise<void> {
    await Promise.all(
      this.componentList.map(async (compId) => {
  ...
  private getRcsbId(
    ...
    else return `${pdb.entryId}${TagDelimiter.entity}${pdb.entityId}`;
  }
  ...
  private componentInfo(
    ...
    const chainInfo: ChainInfo | undefined = this.stateManager.assemblyModelSate
      .getModelChainInfo(this.getRcsbId(data.pdb))
      ?.chains.find(
        (ch) =>
          ("entityId" in data.pdb && ch.entityId == data.pdb.entityId) ||
    ...
    switch (data.tag) {
      case "aligned":
        ...
        const alignedCompId: string = `${data.pdb.entryId}${
          ...
          TagDelimiter.assembly
        }${operatorInfo[0].ids.join(",")}${TagDelimiter.assembly}${"polymer"}`;
      ...
      case "polymer":
        ...
        break;
      case "non-polymer":
        for (const tag of createSelectionExpressions(data.pdb.entryId)
          .map((expression) => expression.tag)
          .filter((tag) => tag != "water" && tag != "polymer")) {
          const nonPolymerCompId: string = `${data.pdb.entryId}${TagDelimiter.entity}${chainInfo.entityId}${TagDelimiter.assembly}${tag}`;
