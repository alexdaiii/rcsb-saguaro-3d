  ...
  implements StructureViewerBehaviourObserverInterface<R, L>
{
  ...
  public observe(
    ...
    this.structureBehaviour = new AssemblyBehaviour(
      structureViewer,
      stateManager,
    );
...
class AssemblyBehaviour<R, L> implements StructureViewerBehaviourInterface {
  ...
  private subscribe(): Subscription {
    return this.stateManager.subscribe<
      "visibility-change" | "component-info",
      {display: "visible" | "hidden"; label: string}
    >(async (o) => {
      ...
      if (o.type == "pfv-change" && o.view == "1d-view") this.resetPluginView();
  ...
  public async featureClick(): Promise<void> {
    ...
    const {modelId, labelAsymId, operatorName, regions} =
      this.stateManager.selectionState.getLastSelection() ?? {};
    if (
      modelId &&
      labelAsymId &&
      operatorName &&
      ...
      else this.selectedComponentId = undefined;
    } else {
      const modelId: string =
        this.stateManager.assemblyModelSate.getString("modelId");
      const labelAsymId: string =
        this.stateManager.assemblyModelSate.getString("labelAsymId");
      const operatorName: string | undefined =
        this.stateManager.assemblyModelSate.getOperator()?.name;
      this.structureViewer.clearSelection("select", {
        modelId,
        labelAsymId,
        operatorName,
      });
  ...
  private select(mode: "select" | "hover", operator: "add" | "set"): void {
    ...
    const operatorName: string | undefined =
      this.stateManager.assemblyModelSate.getOperator()?.name;
    const selection: SaguaroRegionList | undefined =
      this.stateManager.selectionState.getSelectionWithCondition(
        modelId,
        labelAsymId,
        mode,
        operatorName,
  ...
  private componentInfo(data: {label: string}): void {
    this.stateManager.next<
      "component-info",
      {label: string; display: "visible" | "hidden"}
    >({
      type: "component-info",
      view: "3d-view",
      data: {
        label: data.label,
        display: this.structureViewer.displayComponent(data.label)
          ? "visible"
          : "hidden",
      },
    });
