...
} from "../../RcsbFvSequence/RcsbFvSequence";
import {
  RcsbFvStructure,
...
export interface RcsbFv3DComponentProps<T, R, L, S, U> {
...
export class RcsbFv3DComponent<T, R, L, S, U> extends React.Component<
  RcsbFv3DComponentProps<T, R, L, S, U> & { resolve: () => void },
...
> {
  ...
  render(): JSX.Element {
    return (
      <div
        className={
          this.props.fullScreen ? classes.fullScreen : classes.fullHeight
        }
      >
        <div
          id={this.ROOT_DIV_ID}
          style={RcsbFv3DComponent.mainDivCssConfig(
            this.props.cssConfig?.rootPanel,
          )}
          className={this.useDefaultCss() ? classes.rcsbFvMain : ""}
          onMouseMove={(evt: MouseEvent<HTMLDivElement>) => {
            this.mouseMove(evt);
          }}
          onMouseUp={(e) => {
            this.splitPanelMouseUp();
          }}
        >
          <div
            style={this.structureCssConfig(
              this.props.cssConfig?.structurePanel,
            )}
          >
            <RcsbFvStructure<R, L, S>
              ...
              structureViewerBehaviourObserver={
                this.props.structureViewerBehaviourObserver
              }
            />
          </div>
          <div
            style={this.sequenceCssConfig(this.props.cssConfig?.sequencePanel)}
          >
            <RcsbFvSequence<T, U>
              ...
              rcsbViewBehaviour={
                this.props.sequencePanelConfig.rcsbViewBehaviour
              }
            />
          </div>
          {this.panelDelimiter()}
  ...
  private panelDelimiter(): JSX.Element {
    return this.useDefaultCss() ? (
      <div
        ...
        style={{
          right: Math.round((1 - this.state.pfvScreenFraction) * 100) + "%",
        }}
      />
    ) : (
      <></>
  ...
  private structureCssConfig(css: CSSProperties | undefined): CSSProperties {
    const widthFr: number = Math.round(
      (1 - this.state.pfvScreenFraction) * 100,
    );
  ...
  private subscribe(): Subscription {
    return this.props.ctxManager.subscribe(
      (obj: RcsbFvContextManagerInterface<T, R, S, U>) => {
        if (obj.eventType == EventType.UPDATE_CONFIG) {
          ...
          this.props.structureViewer.pluginCall(
            obj.eventData as (f: PluginContext) => void,
          );
        }
      },
    );
  ...
  private updateConfig(config: UpdateConfigInterface<T, R, S, U>) {
    const structureConfig:
      | Partial<RcsbFvStructureConfigInterface<R, S>>
      | undefined = config.structurePanelConfig;
    const sequenceConfig: Partial<RcsbFvSequenceInterface<T, U>> | undefined =
    ...
    if (structureConfig != null && sequenceConfig != null) {
      this.setState({
        ...
        sequencePanelConfig: {
          ...this.state.sequencePanelConfig,
          ...sequenceConfig,
        },
      });
  ...
  private mouseMove(evt: MouseEvent<HTMLDivElement>): void {
    if (typeof this.resize === "function") this.resize(evt);
  }
}
