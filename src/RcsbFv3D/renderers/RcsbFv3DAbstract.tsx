...
export abstract class RcsbFv3DAbstract<T, R, L, S, U> {
  ...
  private readonly structureViewerBehaviourObserver: StructureViewerBehaviourObserverInterface<
    R,
    L
  >;
  private readonly sequenceConfig: RcsbFvSequenceInterface<T, U>;
  private readonly ctxManager: RcsbFvContextManager<T, R, S, U> =
    new RcsbFvContextManager<T, R, S, U>();
  ...
  protected constructor(config: RcsbFv3DAbstractInterface<T, R, L, S, U>) {
    ...
    this.structureViewerBehaviourObserver =
      config.structureViewerBehaviourObserver;
  }

  public async render(): Promise<void> {
    if (this.elementId == null) throw new Error("HTML element not found");
  ...
  public unmount(
    ...
    if (element != null) {
      ...
      if (typeof unmountCallback === "function") unmountCallback();
      this.fullScreen("off");
    }
  }
  ...
  public updateConfig(config: {
    ...
    this.ctxManager.next({
      eventType: EventType.UPDATE_CONFIG,
      eventData: config,
    });
  }
