export class AlignmentManager {
  private readonly targetToQuery: Map<number, number> = new Map<
    number,
    number
  >();
  private readonly queryToTarget: Map<number, number> = new Map<
    number,
    number
  >();
  ...
  public getTargetRange(queryRange: {
    begin: number;
    end: number;
    ...
    while (queryBegin <= queryEnd) {
      ...
      if (out.begin != 0 && out.end != 0) return out;
    }
    return undefined;
  }

  public getQueryRange(targetRange: {
    begin: number;
    end: number;
  }): {begin: number; end: number} | undefined {
