...
export class AlignmentReference {
  ...
  private readonly alignmentMap = new Map<
    string,
    {
      entryId: string;
      instanceId: string;
  ...
  public async init(result: Alignment) {
    ...
    this.alignmentRefMap = Array(length)
      .fill(0)
      .map((v, n) => n + 1);
  }
  ...
  public addAlignment(
    ...
    for (const gapBeg in gaps) {
      ...
      else this.addGap(parseInt(gapBeg), gaps[gapBeg]);
    }
    const beg = alignment.filter(
      (a) => a && this.alignmentRefMap[0] && a < this.alignmentRefMap[0],
    ) as number[];
    if (beg.length > 0) this.addBeg(beg);
    const n = this.alignmentRefMap[this.alignmentRefMap.length - 1] as number;
    const end = alignment.filter((a) => a && n && a > n) as number[];
    if (end.length > 0) this.addEnd(end);
    this.addRef(id, alignment, target);
  }

  public buildAlignments(): AlignmentResponse {
    return buildAlignments(
      this.refId,
  ...
  public addUniqueAlignmentId(
    ...
    const asymId =
      "asym_id" in res.selection ? res.selection.asym_id : undefined;
    if (
      "entry_id" in res &&
      res.entry_id &&
      res.selection &&
    ...
    if (!this.alignmentMap.has(`${entryId}${TagDelimiter.instance}${asymId}`)) {
      ...
      let tag = 1;
      while (
        this.alignmentMap.has(
          `${entryId}[${tag}]${TagDelimiter.instance}${asymId}`,
        )
      ) {
        tag++;
      }
      this.alignmentMap.set(
        `${entryId}[${tag}]${TagDelimiter.instance}${asymId}`,
        {
          entryId,
          instanceId: asymId,
  ...
  public getAlignmentEntry(alignmentId: string): {
    entryId: string;
    instanceId: string;
    sequence: string;
    alignmentIndex: number;
  ...
  public async getSequences(): Promise<InstanceSequenceInterface[]> {
    const out = Array.from(this.alignmentMap.values())
      .filter((v) => v.sequence.length > 0)
      .map((v) => ({
        rcsbId: v.alignmentId,
        sequence: v.sequence,
      }));
    const missingSeq = await RcsbRequestContextManager.getInstanceSequences(
      Array.from(this.alignmentMap.values())
        .filter((v) => v.sequence.length == 0)
        .map((v) => `${v.entryId}${TagDelimiter.instance}${v.instanceId}`)
        ...
        .map((v) => ({
          ...
          sequence:
            missingSeq.find(
              (s) =>
                s.rcsbId ===
                `${v.entryId}${TagDelimiter.instance}${v.instanceId}`,
            )?.sequence ?? "",
        })),
    );
  }
  ...
  private addRef(
    ...
    const map: AlignmentRefType = Array(this.alignmentRefMap.length).fill(
      undefined,
    );
    alignment.forEach((v, n) => {
      if (typeof v === "undefined") return;
      const index = this.alignmentRefMap.findIndex((e) => e === v);
      if (index >= 0) map[index] = n;
    });
    const gaps = findGaps(alignment);
    for (const gapBeg in gaps) {
      const index = this.alignmentRefMap.findIndex(
        (v) => v === parseInt(gapBeg),
      );
...
function buildAlignments(
  refId: string,
  alignmentRefMap: AlignmentRefType,
  alignmentMembers: AlignmentMemberType[],
): AlignmentResponse {
  ...
  alignmentMembers.forEach((am) => {
    out.target_alignment?.push({
      target_id: am.id,
      aligned_regions: buildRegions(
        am.map.map((v, n) =>
          typeof v === "number" ? am.target[v] : undefined,
        ),
      ),
    });
  });
...
function findGaps(alignment: AlignmentRefType): Record<number, number> {
  ...
  alignment.forEach((v, n) => {
    if (!v) {
      if (gapBeg == 0) gapBeg = alignment[n - 1] as number;
      gapLength++;
    } else {
      if (gapLength > 0) {
  ...
  if (gapLength > 0) out[gapBeg] = gapLength;
  return out;
}
